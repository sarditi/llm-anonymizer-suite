...
7.
8. The extension is polling that directory (fs.watch + 1.5 s interval). When the file appears and is size-stable, it `chmod +x` it and spawns it.
9. Standard output is streamed line-by-line into the session view with a color scheme:
...
10. After the script exits, a **Rollback** button is enabled. It runs the same script with `--rollback`. If more than 60 minutes have passed, the script's own `=== ERROR: Rollback window has expired. ===` message is surfaced as-is (the extension does not reimplement the rollback logic).
